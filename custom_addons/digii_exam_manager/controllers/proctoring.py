...
class ProctoringController(http.Controller):
    ...
    @http.route('/exam/proctoring/upload-chunk',
                type='http', auth='user', methods=['POST'], csrf=False)
    def upload_chunk(self, session_id=None, index=None, chunk=None, **kw):
        ...
        if idx < 0 or not chunk:
            return self._json_response({'error': 'Parametres manquants'}, 400)
        try:
            session.store_video_chunk(idx, chunk.read())
        except UserError as exc:
            return self._json_response({'error': str(exc)}, 409)
